...
def main():
    ...
    csv_path = os.path.join(current_dir, '../data/clientes.csv')
    ...
    # Codificar la columna 'Tipo de Cliente' a valores numéricos
    df['Tipo de Cliente'] = df['Tipo de Cliente'].apply(lambda x: 1 if x == 'Leal' else 0)

    # Dividir los datos en características (X) y etiquetas (y)
    X = df[['Numero de Reservas']]
    y = df['Tipo de Cliente']
    ...
    # Visualizar el árbol de decisión con las variables de predicción
    plt.figure(figsize=(10, 8))
    plot_tree(clf, feature_names=list(X.columns), filled=True, rounded=True, class_names=['No Leal', 'Leal'], impurity=False)
    plt.tight_layout()  # Ajustar el diseño para una mejor visualización
    plt.savefig('../outputs/decision_tree_cli.png')
    plt.show()

    print("Árbol de decisión generado como '../outputs/decision_tree_cli.png'")
